...
const Sidebar: React.FC = () => {
  const { openSidebar, toggleSidebar } = useContext(ThemeContext);

  return (
    <div className={`header-right-sidebar ${openSidebar ? "isActive" : ""}`}>
      <div
        className='header-right-sidebar__overlay'
        onClick={toggleSidebar}
      ></div>
      <div className='header-right-sidebar__content'>
        <span className='header-right-sidebar__close' onClick={toggleSidebar}>
        ...
        <div className='header-right-sidebar__logo-box'>
          <Link to='/' aria-label='logo image'>
            ...
            <img src={sidebarData.logo} width={158} height={50} alt='logo' />
          </Link>
        </div>
